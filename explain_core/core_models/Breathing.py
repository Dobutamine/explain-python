...
class Breathing(BaseModel):
    ...
    minute_volume_ref: float = 0.64  # in L/min/kg
    vt_rr_ratio: float = 0.03  # in L/bpm/kg
    ...
    target_minute_volume: float = 0.4  # in L/min
    ...
    def calc_model(self) -> None:
        ...
        _minute_volume_ref = self.minute_volume_ref * _weight
        ...
        if self._exp_timer > self._te:
            ...
            if self.is_intubated:
                self.exp_tidal_volume = self._model.models[
                    "Ventilator"
                ].exp_tidal_volume
            else:
    ...
    def calc_resp_muscle_pressure(self) -> float:
        ...
        # inspiration
        if self._insp_running:
            mp = (self._ncc_insp / (self._ti / self._t)) * self._rmp_gain
