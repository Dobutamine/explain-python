...
class Ans(BaseModel):
    ...
    def calc_model(self) -> None:
        ...
        if self._update_counter > self._update_window:
            ...
            self.hr_effect_factor = self._hr_effector.calc_ans_effect_factor(
                self._map, self._pco2, self._ph, self._po2
            )
            for hrt in self._hr_targets:
                hrt.hr_ans_factor = self.hr_effect_factor

            self.mv_effect_factor = self._mv_effector.calc_ans_effect_factor(
                self._map, self._pco2, self._ph, self._po2
            )
            for mvt in self._mv_targets:
                mvt.mv_ans_factor = self.mv_effect_factor

            self.venpool_effect_factor = self._venpool_effector.calc_ans_effect_factor(
                self._map, self._pco2, self._ph, self._po2
            )
            for vpt in self._venpool_targets:
                vpt.u_vol_ans_factor = self.venpool_effect_factor

            self.cont_effect_factor = self._cont_effector.calc_ans_effect_factor(
                self._map, self._pco2, self._ph, self._po2
            )
            for contt in self._cont_targets:
                contt.el_max_ans_factor = self.cont_effect_factor

            self.svr_effect_factor = self._svr_effector.calc_ans_effect_factor(
                self._map, self._pco2, self._ph, self._po2
            )
            for svrt in self._svr_targets:
                svrt.r_ans_factor = self.svr_effect_factor

            self.pvr_effect_factor = self._pvr_effector.calc_ans_effect_factor(
                self._map, self._pco2, self._ph, self._po2
            )
...
class AnsTarget:
    ...
    def calc_gains(self):
        self.g_map_low = self.translate_mxe(self.mxe_map_low) / (
            self.min_map - self.set_map
        )
        self.g_map_high = self.translate_mxe(self.mxe_map_high) / (
            self.max_map - self.set_map
        )

        self.g_pco2_low = self.translate_mxe(self.mxe_pco2_low) / (
            self.min_pco2 - self.set_pco2
        )
        self.g_pco2_high = self.translate_mxe(self.mxe_pco2_high) / (
            self.max_pco2 - self.set_pco2
        )

        self.g_ph_low = self.translate_mxe(self.mxe_ph_low) / (
            self.min_ph - self.set_ph
        )
        self.g_ph_high = self.translate_mxe(self.mxe_ph_high) / (
            self.max_ph - self.set_ph
        )

        self.g_po2_low = self.translate_mxe(self.mxe_po2_low) / (
            self.min_po2 - self.set_po2
        )
        self.g_po2_high = self.translate_mxe(self.mxe_po2_high) / (
            self.max_po2 - self.set_po2
        )

    def calc_ans_effect_factor(
        self, _map: float, _pco2: float, _ph: float, _po2: float
    ) -> float:
        # calculate the activation functions
        self._a_map = activation_function(
            _map, self.max_map, self.set_map, self.min_map
        )
