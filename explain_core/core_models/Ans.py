...
class Ans:
    ...
    def init_model(self, **args: dict[str, any]):
        ...
        for effector_name, effector in self.effectors.items():
            ...
            self._effectors[effector_name] = {
                ...
                "cum_weight": 0.0,
                "tc": effector["tc"],
                "effect_factor": effector["effect_factor"],
            }
    ...
    def calc_model(self):
        ...
        for _effector_name, _effector in self._effectors.items():
            ...
            _effector_factor = 1.0
            if _firing_rate_avg > 50.0:
                _effector_factor = 1 + (_effector["cum_mxe_high"] - 1) / 50.0 * (
                    _firing_rate_avg - 50.0
                )
            else:
                _effector_factor = _effector["cum_mxe_low"] + (
                    1 - _effector["cum_mxe_low"]
                ) / 50.0 * (_firing_rate_avg)

            # 0.1 -> -0.9   0.9 -> -0.1
            if _effector_factor < 1.0:
                _effector_factor = -(1 - _effector_factor)
            else:
                _effector_factor = _effector_factor - 1.0
            # transfer the effect factor to the target model
            setattr(
                _effector["target_model"], _effector["target_prop"], _effector_factor
            )
