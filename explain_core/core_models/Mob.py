...
class Mob(BaseModel):
    ...
    hw: float = 21.9  # heart weight = 7.799 + 0.004296 * birth weight (grams)
    bm_vo2_ref: float = 0.0001590  # in ml O2/cardiac cycle/gram heart_weight: vo2 when the heart is not beating
    bm_vo2_max: float = (
        0.0001590  # maximal vo2 in ml O2/cardiac cycle/gram heart_weight
    )
    bm_vo2_min: float = (
        0.0000318  # minimal vo2 in ml O2/cardiac cycle/gram heart_weight
    )
    ecc_c: float = 0.0  # not implemented yet but included in basal metabolism
    pva_c: float = 0.000194  # CPVA in mL O2/gram/heart weight/cardiac cycle/mmHg*l
    ...
    def init_model(self, model: object) -> bool:
        ...
        self.hw = (
            7.799 + 0.004296 * self._model.weight * 1000.0
        )  # = 21.9 grams for a 3 kg baby
    ...
    def oxygen_metabolism(self) -> float:
        ...
        self._a_po2 = activation_function(
            po2_cor, self.po2_max, self.po2_set, self.po2_min
        )
        ...
        self.bm_g = (self.bm_vo2_max * self.hw - self.bm_vo2_min * self.hw) / (
            self.po2_max - self.po2_min
        )
        ...
        self._d_bm_vo2 = (
            self._t * ((1 / self.bm_tc) * (-self._d_bm_vo2 + self._a_po2))
            + self._d_bm_vo2
        )
        self._d_hr = (
            self._t * ((1 / self.hr_tc) * (-self._d_hr + self._a_po2)) + self._d_hr
        )
        self._d_cont = (
            self._t * ((1 / self.cont_tc) * (-self._d_cont + self._a_po2))
            + self._d_cont
        )
        self._d_ans = (
            self._t * ((1 / self.ans_tc) * (-self._d_ans + self._a_po2)) + self._d_ans
        )

        # calculate the baseline vo2 in mmol O2 /  cardiac cycle
        self.bm_vo2 = (
            (self.bm_vo2_ref * self.hw) + self._d_bm_vo2 * self.bm_g
        ) / self._ml_to_mmol
        ...
        self.ecc_vo2 = self.ecc * self.ecc_c * self.hw

        # calculate the pva vo2 in mmol O2 / cardiac cycle
        self.pva_vo2 = (self.pva * self.pva_c * self.hw) / self._ml_to_mmol
    ...
    def calc_pva(self) -> float:
        ...
        self._pv_area_lv += (_dV_lv * self._prev_lv_pres) + (
            _dV_lv * (self._heart._lv.pres - self._prev_lv_pres)
        ) / 2.0
        if _dV_lv > 0:
            self._sv_lv_cum += _dV_lv

        _dV_rv = self._heart._rv.vol - self._prev_rv_vol
        self._pv_area_rv += (_dV_rv * self._prev_rv_pres) + (
            _dV_rv * (self._heart._rv.pres - self._prev_rv_pres)
        ) / 2.0
        if _dV_rv > 0:
