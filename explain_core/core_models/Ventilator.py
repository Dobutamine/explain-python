...
class Ventilator(BaseModel):
    ...
    def calc_model(self):
        ...
        if self._exp_time_counter > self.exp_time or self._triggered_breath:
            ...
            if self.exp_tidal_volume > 0:
                self.elastance = (
                    self._pip - self._peep
                ) / self.exp_tidal_volume  # in mmHg/l
                self.compliance = 1 / (
        ...
        self.vc_po2 = self._ventcircuit.po2
        self.vc_pco2 = self._ventcircuit.pco2
    ...
    def pressure_control(self):
        if self._inspiration:
            ...
            self._insp_valve.r_for = (
                self._ventin.pres + self._pip - self.p_atm - self._peep
            ) / (self.insp_flow / 60.0)
            ...
            if self._ventcircuit.pres > self._pip + self.p_atm:
                ...
                if self._insp_valve.flow > 0 and not self._pres_reached:
                    self.resistance = (
                        self._ventcircuit.pres - self.p_atm
    ...
    def build_ventilator(self, model):
        ...
        self._tube_resistance = get_ettube_resistance(
            self.ettube_diameter, self.insp_flow
        )
