...
class Ecls(BaseModel):
    ...
    ecls_running: bool = (
        False  # determines whether the artificial whomb is running or not
    )
    ...
    pat_sample_site: str = "AD"  # site where patient bloodgas is sampled
    ...
    drainage_tubing_length: float = (
        100  # total length of the ecls drainage tubing in mm
    )
    ...
    p_ven: float = 0.0  # venous inlet pressure
    p_int: float = 0  # pre oxygenator pressure
    p_art: float = 0  # post oxygenator pressure
    ...
    blood_flow: float = 0.0  # mean ecmo blood flow
    blood_flow_instant: float = 0.0  # instantaneous blood flow
    ...
    # ecls parts
    _pat_sample_site: BloodCapacitance = {}
    _ecls_parts = []  # define a list holding all ecls system parts
    _tubing_in: BloodCapacitance = (
        {}
    )  # blood containing ecls tubing between drainage site and the pump
    _oxy: BloodCapacitance = {}  # blood in the oxygenator
    _pump: BloodPump = {}  # ecls pump
    _tubing_out: BloodCapacitance = (
        {}
    )  # blood containing ecls tubing between oxygenator and the return site

    _drainage_tubing_in: Resistor = (
        {}
    )  # resistor connecting the umbilical arteries to the ecls tubing
    ...
    _oxy_tubing_out: Resistor = (
        {}
    )  # resistor connecting the oxygenator to the ecls tubing
    _tubing_out_return: Resistor = (
        {}
    )  # resistor connecting the ecls tubing to the umbilical veins
    ...
    _oxy_gas_in: GasCapacitance = (
        {}
    )  # gas capacitance holding the air going into the oxygenator
    _oxy_gas_out: GasCapacitance = (
        {}
    )  # gas capacitance where the air is going into after the oxugenator
    ...
    _bg_eval_interval: float = (
        1.0  # interval at which the bloodgas composition is calculated
    )
    _bg_eval_counter: float = 0.0  # counter for the bloodgas composition

    _flows: float = []
    _sum_flows: float = 0.0
    _data_window: int = 4000
    ...
    def init_model(self, model: object) -> bool:
        ...
        # get the patient sample sit
        self._pat_sample_site = self._model.models[self.pat_sample_site]

        # fill the list of flows
        self._flows = [0.0] * self._data_window
    ...
    def calc_model(self) -> None:
        if self.ecls_running:
            ...
            sg_ls: float = (
                self.oxy_sweep_gas / 60.0
            ) + co2_ls  # in l/s    = sweep gas flow
            r: float = (self._oxy_gas_in.pres - self._oxy_gas_out.pres) / sg_ls
            ...
            self.fico2_gas = (co2_ls / sg_ls) + fico2_sg
            ...
            if self._bg_eval_counter > self._bg_eval_interval:
                ...
                set_blood_composition(self._pat_sample_site)

            self._bg_eval_counter += self._t

    def diagnostics(self) -> None:
        # pre and post oxygenator bloodgasses
        self.pre_oxy_bg["ph"] = self._tubing_in.aboxy["ph"]
        self.pre_oxy_bg["pco2"] = self._tubing_in.aboxy["pco2"] * self._mmhg_kpa
        self.pre_oxy_bg["po2"] = self._tubing_in.aboxy["po2"] * self._mmhg_kpa
        self.pre_oxy_bg["so2"] = self._tubing_in.aboxy["so2"]
        self.pre_oxy_bg["hco3"] = self._tubing_in.aboxy["hco3"]
        self.pre_oxy_bg["be"] = self._tubing_in.aboxy["be"]

        self.post_oxy_bg["ph"] = self._tubing_out.aboxy["ph"]
        self.post_oxy_bg["pco2"] = self._tubing_out.aboxy["pco2"] * self._mmhg_kpa
        self.post_oxy_bg["po2"] = self._tubing_out.aboxy["po2"] * self._mmhg_kpa
        self.post_oxy_bg["so2"] = self._tubing_out.aboxy["so2"]
        self.post_oxy_bg["hco3"] = self._tubing_out.aboxy["hco3"]
        self.post_oxy_bg["be"] = self._tubing_out.aboxy["be"]

        self.pat_bg["ph"] = self._pat_sample_site.aboxy["ph"]
        self.pat_bg["pco2"] = self._pat_sample_site.aboxy["pco2"] * self._mmhg_kpa
        self.pat_bg["po2"] = self._pat_sample_site.aboxy["po2"] * self._mmhg_kpa
        self.pat_bg["so2"] = self._pat_sample_site.aboxy["so2"]
        self.pat_bg["hco3"] = self._pat_sample_site.aboxy["hco3"]
        self.pat_bg["be"] = self._pat_sample_site.aboxy["be"]

        # ecmo pressures
        self.p_ven = self._tubing_in.pres
        self.p_int = self._oxy.pres
        self.p_art = self._tubing_out.pres
        self.tmp = self.p_int - self.p_art

        # ecmo flow
        self._flows.insert(0, self._drainage_tubing_in.flow * 60.0)
        self.blood_flow_instant = self._drainage_tubing_in.flow * 60.0

        # remove the last flows from the list
        removed_value = self._flows.pop()
        self._sum_flows -= removed_value
        self._sum_flows += self._drainage_tubing_in.flow * 60.0

        # get the moving average of the flow.
        self.blood_flow: float = self._sum_flows / self._data_window
    ...
    def build_ecls(self, model) -> None:
        ...
        self.tubing_in_volume = (
            math.pi
            * math.pow(self.tubing_diameter / 2000, 2)
            * self.drainage_tubing_length
        )
        ...
        self.tubing_out_volume = (
            math.pi
            * math.pow(self.tubing_diameter / 2000, 2)
            * self.return_tubing_length
        )
